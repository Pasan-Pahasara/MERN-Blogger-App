...
export default class CommentController { // CommentController is the controller of the comment
  createComment: RequestHandler = async ( // createComment is the function to create a comment
    req: Request,
    res: Response
    ...
    try {
      ...
      if (!createComment) { // save comment only the comment  name is not existing
        ...
        return res // return the response
          .status(200)
          .json({ message: "New comment added.!", responseData: createComment }); 
      } else {
  ...
  retrieveAllComments: RequestHandler = async ( // retrieveAllComments is the function to retrieve all the comments
    req: Request,
    res: Response
  ): Promise<Response> => { // Promise<Response> is the return type of the function
    try {
  ...
  updateComment: RequestHandler = async ( // updateComment is the function to update a comment
    req: Request,
    res: Response
  ): Promise<Response> => { // Promise<Response> is the return type of the function
    try {
      ...
      let updatedComment = await Comment.findByIdAndUpdate(id, req.body, { // findByIdAndUpdate() is used to find the document by id and update
        new: true,
      });
      return res // return the response
        .status(200)
  ...
  deleteComment = async (req: Request, res: Response): Promise<Response> => { // deleteComment is the function to delete a comment
    try {
      // destructuring assignment
      const { id } = req.params; // id is the id of the comment
      ...
      if (!deletedComment) { // check whether the comment is deleted or not
